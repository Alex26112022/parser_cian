...
class ParserCianNew:
    ...
    def get_total_pages(self):
        """ Определяет количество страниц для пагинации. """
        response = requests.get(self.__url, headers=self.__headers,
                                params=self.params)

        soup = BeautifulSoup(response.text, 'lxml')
        total_cards_ = soup.find('h5').text.split()[1:-1]
        if len(total_cards_) > 1:
            total_cards_ = int(''.join(total_cards_))
        else:
            total_cards_ = int(total_cards_[0])
        self.total_cards = total_cards_
        print(total_cards_)
        self.total_pages = int(self.total_cards) // 28
        print(self.total_pages)

    def get_response(self):
        for page_ in range(1, self.total_pages + 1):
            response = requests.get(self.__url, headers=self.__headers,
                                    params=self.params)
